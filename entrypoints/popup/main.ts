...
root.innerHTML = `
  <div class="panel">
    <header class="panel__header">
      ...
      <a href="https://apps.apple.com" target="_blank" rel="noreferrer" data-i18n="headerLinkLabel"></a>
    </header>
...
const elements = {
  currentRegionValue: root.querySelector<HTMLSpanElement>('#current-region-value')!,
  switchStatus: root.querySelector<HTMLParagraphElement>('#switch-status')!,
  overlayToggle: root.querySelector<HTMLInputElement>('#overlay-toggle')!,
  saveButton: root.querySelector<HTMLButtonElement>('#save-button')!,
  saveStatus: root.querySelector<HTMLParagraphElement>('#save-status')!,
  languageSelect: root.querySelector<HTMLSelectElement>('#language-select')!,
  searchInput: root.querySelector<HTMLInputElement>('#search-input')!,
  regionsList: root.querySelector<HTMLDivElement>('#regions-list')!,
};
...
function updateSwitchState() {
  ...
  if (!tabUrl || !isAppleAppStoreUrl(tabUrl)) {
    ...
    setSwitchStatus('statusOpenAppStore');
    return;
  }
...
function renderRegionsList() {
  ...
  applyLanguageTexts(); // To translate the group titles
}
...
function updateDirtyState() {
  ...
  state.dirty = overlayChanged || favoritesChanged || languageChanged;
  updateSaveButtonState();
}
...
function refreshStatuses() {
  const { switch: switchStatus, save: saveStatus } = state.status;
...
function applyLanguageTexts() {
  ...
  root.querySelectorAll<HTMLOptionElement>('[data-i18n-option]').forEach((option) => {
